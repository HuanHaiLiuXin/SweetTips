...
package com.jet.sweettips.snackbar;
...
/**
 * Manages {@link SweetSnackbar}s.
 */
class SweetSnackbarManager {
    ...
    private static SweetSnackbarManager sSnackbarManager;

    static SweetSnackbarManager getInstance() {
        if (sSnackbarManager == null) {
            sSnackbarManager = new SweetSnackbarManager();
        }
    ...
    void handleTimeout(SnackbarRecord record) {
        synchronized (mLock) {
            if (mCurrentSnackbar == record || mNextSnackbar == record) {
                cancelSnackbarLocked(record, SweetSnackbar.Callback.DISMISS_EVENT_TIMEOUT);
            }
        }
    }
